...
export async function removeMultipleRolesForUser(guild_id: string, discord_user_id: string, discord_roles: string[]) {
	...
	member.roles.remove(discord_roles).catch((error) => {
		console.log(error);
	});
}
